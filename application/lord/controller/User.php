...
public function delQianyue(){
    ...
    public function addprice()
    {
        ...
        $post = input('post.');
        if (!$post || empty($post['uid']) || !is_numeric($post['uid']) || $post['uid'] <= 0) {
            return WPreturn('UID người dùng không hợp lệ', -1);
        }
        if (empty($post['bpprice']) || !is_numeric($post['bpprice']) || $post['bpprice'] <= 0) {
            return WPreturn('Số tiền phải là số dương lớn hơn 0', -1);
        }
        ...
        try {
            ...
            $user = Db::name('userinfo')->lock(true)->where('uid', $post['uid'])->find();
            if (!$user) {
                throw new \Exception("Người dùng không tồn tại");
            }

            // Tính số dư mới từ usermoney hiện tại
            $newBalance = $user['usermoney'] + $post['bpprice'];
            ...
            $b_data = [
                ...
                'bptime' => time(),
                'cltime' => time(),
                'bpprice' => $post['bpprice'],
                'remarks' => 'Admin ID ' . $_SESSION['userid'] . ' cộng tiền thủ công',
                'uid' => $user['uid'],
                'isverified' => 1,
                'bpbalance' => $newBalance
            ];
            Db::name('balance')->insert($b_data);

            // Update số dư userinfo
            Db::name('userinfo')->where('uid', $post['uid'])->update(['usermoney' => $newBalance]);
            ...
            Db::rollback();
            return WPreturn('Thao tác thất bại: ' . $e->getMessage(), -1);
        }
    }
