...
{
    $data = $request->post();
    $conversation_id = isset($data['conversation_id']) ? $data['conversation_id'] : 0;
    $user_id = isset($data['user_id']) ? intval($data['user_id']) : 0; // user gửi tin
    ...
    if (!$conversation_id && $user_id) {
        ...
        if ($conversation) {
            ...
            $conversation_id = Db::table('wp_chat_conversations')->insertGetId([
                'user_id' => $user_id,
                'admin_id' => 10003, // hoặc $_SESSION['uid'] nếu là admin hiện tại
    ...
    $message_id = Db::table('wp_chat_messages')->insertGetId([
        ...
        'sender_id' => 0, // 0 nghĩa là admin, nếu user gửi thì để $user_id
        'content' => $content,
    ...
    return json(['status' => 1, 'msg' => 'Sent', 'conversation_id' => $conversation_id]);
}
