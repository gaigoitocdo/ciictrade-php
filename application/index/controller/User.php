...
public function cpaypassword()
{
    ...
    $upwd2    = input('post.upwd2');

    if (!$nickname || !$upwd || !$upwd2) {
        return json(['type'=>0, 'data'=>'Vui lòng nhập đầy đủ thông tin!']);
    }
    if ($upwd !== $upwd2) {
        return json(['type'=>0, 'data'=>'Mật khẩu xác nhận không khớp!']);
    }

    $user = Db::name('userinfo')
        ->where('username', $nickname)
        ->whereOr('utel', $nickname)
        ->find();

    if (!$user) {
        return json(['type'=>0, 'data'=>'Không tìm thấy tài khoản!']);
    }

    $hashed = md5($upwd);
    Db::name('userinfo')
        ->where('uid', $user['uid'])
        ->update(['paypwd' => $hashed]);

    return json(['type'=>1, 'data'=>'Đổi mật khẩu thanh toán thành công!']);
}
...
public function showCpaypassword() {
    return $this->fetch('user/cpaypassword');
}
